...
/**
 * This class is used to test a Big Two card game. It stores the a deck of cards, 
 ...
 *
 */
public class Tester extends BigTwo
{
	/**
		...
		for(int i=0;i<playerList.size();i++)
		{
			for(int j=0;j<13;j++)
			{
				...
				if(card.getRank()==2 && card.getSuit()==0)
					currentIdx = i; //the one with Diamond 3 is the first player
			}
		}
		for(int i=0;i<playerList.size();i++)
		{
